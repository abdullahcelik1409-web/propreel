...
export async function transformImageToVerticalCanvas(imageBuffer, {
  ...
  try {
    ...
    const blurredBackground = original.clone();
    blurredBackground.cover({ w: canvasWidth, h: canvasHeight });
    blurredBackground.blur(40);

    const fittedForeground = original.clone();
    fittedForeground.contain({
      w: canvasWidth,
      h: canvasHeight,
      color: 0x00000000,
    ...
    blurredBackground.composite(fittedForeground, 0, 0);
    return Buffer.from(await blurredBackground.getBuffer(PROCESSED_IMAGE_CONTENT_TYPE));
  } catch (error) {
