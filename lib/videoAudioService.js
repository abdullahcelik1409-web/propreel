...
import { getAudioTrackById } from "./audioTrackService.js";
...
async function runFfmpeg(args, { allowFailure = false } = {}) {
  const ffmpegPath = await getFfmpegPath({ purpose: "audio merge" });
...
export async function addBackgroundAudioToFinalVideo({ video, silentVideoUrl }) {
  ...
  const workDir = join(tmpdir(), `viseo-audio-${video.id || Date.now()}`);
