...
export async function submitMultiImageVideoGeneration({
  ...
  for (let index = 0; index < scenePlan.length; index += 1) {
    ...
    const continuityDirection = [
      ...
      normalizedAspectRatio === "9:16" ? "Composition direction: create a vertical 9:16 mobile-first video. Preserve the centered foreground image and use the prepared vertical reference framing." : null,
    ].filter(Boolean).join("\n");
