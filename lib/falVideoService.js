...
export async function submitMultiImageVideoClip({
  ...
  assertFalCredentials();
  const input = buildFalMultiImageVideoInput({ imageUrl, imageUrls, prompt, durationSeconds, aspectRatio });
...
export async function submitMultiImageVideoGeneration({
  ...
  const normalizedOutputAspectRatio = normalizeAspectRatio(aspectRatio || DEFAULT_VIDEO_ASPECT_RATIO);
  const publicImageUrls = imageUrls.map(assertPublicImageUrl).slice(0, MULTI_IMAGE_MAX_IMAGES);
  const referenceMap = new Map(referenceImages.map((reference) => [reference.originalUrl, reference]));
  ...
  for (let index = 0; index < scenePlan.length; index += 1) {
    ...
    const reference = referenceMap.get(clipImageUrl) || null;
    const clipAspectRatio = reference?.providerAspectRatio || normalizedOutputAspectRatio;
    const requiresVerticalCanvas = Boolean(reference?.requiresVerticalCanvas);
    const continuityDirection = [
      ...
      clipAspectRatio === "1:1" ? "Composition direction: favor a balanced square composition with the main property features centered and protected from edge cropping." : null,
      clipAspectRatio === "9:16" ? "Composition direction: create a native vertical 9:16 mobile-first video from this portrait reference. Preserve the architecture and use the full portrait frame." : null,
      requiresVerticalCanvas ? "Composition direction: create a clean horizontal 16:9 property clip. The application will preserve the full horizontal scene inside a vertical blurred-background canvas after generation." : null,
    ].filter(Boolean).join("\n");
    ...
    providerRequests.push({
      ...
      model: FAL_MULTI_IMAGE_VIDEO_MODEL_ID,
      imageUrl: clipImageUrl,
      requestedAspectRatio: clipAspectRatio,
      outputAspectRatio: normalizedOutputAspectRatio,
      sourceOrientation: reference?.orientation || null,
      sourceWidth: reference?.width || null,
      sourceHeight: reference?.height || null,
