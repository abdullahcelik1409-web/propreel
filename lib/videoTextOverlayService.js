...
export function buildOverlayFilterGraph(overlays, placements, dimensions = null) {
  const canvasFilter = dimensions
    ...
    : null;
  const filters = [`[0:v]${canvasFilter ? `${canvasFilter},` : ""}fps=30,format=yuv420p[base]`];
  let baseLabel = "base";
...
export async function addTextOverlaysToFinalVideo({ video, silentVideoUrl }) {
  ...
  if (!plan?.overlays?.length) {
    return { finalVideoUrl: silentVideoUrl, overlays: video.overlays, applied: false };
  }
  ...
  try {
    ...
    const filterGraph = buildOverlayFilterGraph(plan.overlays, placements, dimensions);
    await runFfmpeg([
