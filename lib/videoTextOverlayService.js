...
export function normalizeOverlayForRender(overlay, aspectRatio) {
  const isCta = Boolean(
    overlay.isCta ||
    overlay.primaryContentSlot === "cta" ||
    (overlay.position === "lower-center" && overlay.align === "center"),
  );
  if (!isCta) return overlay;
  const ctaLayout = aspectRatio === "9:16"
    ? { yPercent: 82, maxWidth: 76, primaryScale: 0.95, safeAreaY: 12 }
    : aspectRatio === "1:1"
      ? { yPercent: 86, maxWidth: 64, primaryScale: 0.88, safeAreaY: 10 }
      : { yPercent: 88, maxWidth: 46, primaryScale: 0.78, safeAreaY: 8 };
  return {
    ...overlay,
    isCta: true,
    primaryContentSlot: overlay.primaryContentSlot || "cta",
    layout: { ...overlay.layout, ...ctaLayout },
  };
}

export function buildOverlaySvg(overlay, videoWidth) {
  const isCta = overlay.isCta || overlay.primaryContentSlot === "cta";
  ...
  return {
    ...
    svg: Buffer.from(`<svg width="${cardWidth}" height="${cardHeight}" viewBox="0 0 ${cardWidth} ${cardHeight}" xmlns="http://www.w3.org/2000/svg">
      <rect x="2" y="2" width="${cardWidth - 4}" height="${cardHeight - 4}" rx="${Math.round(primaryFontSize * 0.25)}" fill="#031010" fill-opacity="${isCta ? Math.min(colors.panelOpacity + 0.14, 0.82) : colors.panelOpacity}" stroke="${colors.accent}" stroke-opacity="${isCta ? 0.78 : 0}" stroke-width="${isCta ? 3 : 0}"/>
...
export function getOverlayCoordinates(overlay, dimensions, card) {
  ...
  const safeX = Math.round(dimensions.width * Number(overlay.layout?.safeAreaX || 0) / 100);
  const safeY = Math.round(dimensions.height * Number(overlay.layout?.safeAreaY || 0) / 100);
  const maxX = Math.max(safeX, dimensions.width - card.width - safeX);
  const maxY = Math.max(safeY, dimensions.height - card.height - safeY);
  return {
    x: Math.min(Math.max(safeX, x), maxX),
    y: Math.min(Math.max(safeY, y), maxY),
  };
}
...
export async function addTextOverlaysToFinalVideo({ video, silentVideoUrl }) {
  ...
  const renderOverlays = plan.overlays.map((overlay) => normalizeOverlayForRender(overlay, plan.aspectRatio));
  ...
  try {
    ...
    for (let index = 0; index < renderOverlays.length; index += 1) {
      const overlay = renderOverlays[index];
    ...
    const filterGraph = buildOverlayFilterGraph(renderOverlays, placements, dimensions, plan.duration);
    await runFfmpeg([
