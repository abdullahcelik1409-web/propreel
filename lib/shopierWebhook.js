...
function parseStructuredString(value) {
  ...
  try {
    ...
    const hasKnownQueryField = /(?:^|&)(orderid|order_id|orderId|productid|product_id|productId|productlist|chartdetails|email|currency|price|buyername|buyersurname|productcount|customernote|istest|res|hash)=/i.test(normalized);
    if (!hasKnownQueryField) return null;
