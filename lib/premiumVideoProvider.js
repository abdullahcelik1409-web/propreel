...
export class FalKling3ProProvider {
  ...
  async generateSceneClip({ scene, startFrameUrl }) {
...
export async function submitPremiumVideoGeneration({
  ...
  for (const scene of scenePlan.scenes) {
    ...
    const bridgeFrameRef = await bridgeFrameService.extractLastFrame(generatedClip, {
      jobId,
      sceneIndex: scene.sceneIndex,
      fallbackUrl: scene.targetPhotoUrl || scene.sourcePhotoUrl,
    });
    previousBridgeFrame = await bridgeFrameService.persistBridgeFrameRef(jobId, scene.sceneIndex, bridgeFrameRef);
    ...
    providerRequests.push({
      ...
      startFrameUrl: startFrame.url,
      bridgeFrameRef: previousBridgeFrame,
