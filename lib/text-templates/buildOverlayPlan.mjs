...
function getPositionLayout(aspectRatio, position, { isCta = false } = {}) {
  ...
  return {
    ...ratio,
    ...(isCta ? ctaLayout : null),
    xPercent: centered ? 50 : ratio.safeAreaX,
    yPercent: isCta ? ctaLayout.yPercent : y,
...
export function buildOverlayPlan({
  ...
  durations.forEach((sceneDuration, sceneIndex) => {
    ...
    let primaryContent = takeUniqueContent(candidates.primary, content, usedText);
    if (!primaryContent) primaryContent = takeUniqueContent(FALLBACK_SLOT_ORDER.filter((slot) => slot !== "cta"), content, usedText);
    if (!primaryContent && sceneIndex === durations.length - 1 && content.cta) {
      primaryContent = { value: content.cta, slot: "cta" };
    }
    if (!primaryContent) return;

    const secondaryContent = takeUniqueContent(candidates.secondary, content, usedText);
    const isCta = primaryContent.slot === "cta";
    ...
    overlays.push({
      ...
      primaryText: primaryContent.value,
      secondaryText: secondaryContent?.value || null,
      primaryContentSlot: primaryContent.slot,
      secondaryContentSlot: secondaryContent?.slot || null,
      isCta,
      ...
      layout: getPositionLayout(normalizedAspectRatio, sourcePlan.position, { isCta }),
    });
  });
