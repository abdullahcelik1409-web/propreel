  ...
  callbacks: {
    ...
    async jwt({ token, user }) {
      if (user) {
        ...
        token.credits = user.credits ?? token.credits ?? 0;
        token.role = user.role || token.role || "user";
      }

      const email = user?.email || token.email;
      const shouldRefreshFromDb = Boolean(email && (user || !token.id || !token.role));
      ...
      if (shouldRefreshFromDb) {
        try {
          ...
          console.error("[auth:jwt] failed to refresh user claims", {
