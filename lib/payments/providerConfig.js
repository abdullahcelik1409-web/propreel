...
export function getActivePaymentProvider(env = process.env) {
  const provider = normalizeProvider(env.PAYMENT_PROVIDER || "lemon");
...
export const paymentProviderConfig = {
  ...
  polar: {
    ...
    footerPaymentText: "Secure USD payments via Polar - hosted checkout for digital credits",
    pricingPaymentNote: "Payments are securely processed by Polar. Polar may act as Merchant of Record where applicable.",
    supportedUseCaseNote: "Best for hosted checkout with Polar products while Viseo keeps credit delivery in its own ledger.",
    receiptText: "Open Viseo",
    customerSupportText: "For payment, receipt, tax, or refund questions, contact Viseo support first; Polar may handle checkout-level records where applicable.",
    ...morText("Polar"),
    legalOverrides: {},
  },
  paytr: {
