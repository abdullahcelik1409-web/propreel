...
export async function createPaymentCheckout({ user, packageId, headers, metadata = {}, provider = getActivePaymentProvider(), env = process.env }) {
  ...
  const result = await adapter.createCheckout({
    ...
    cancelUrl: `${baseUrl}/pricing?checkout=closed`,
    headers,
    metadata,
    env,
  });
...
export async function createPaymentCustomerPortalSession({ user, returnUrl, provider = "polar", env = process.env }) {
  const resolvedProvider = resolvePaymentProvider(provider, env);
  const adapter = getPaymentAdapter(resolvedProvider);
  if (typeof adapter.getCustomerPortalUrl !== "function") {
    throw paymentError(`${getPaymentProviderConfig(resolvedProvider).displayName} customer portal is not configured.`, 501);
  }

  const baseUrl = getAppUrl(env).replace(/\/$/, "");
  return adapter.getCustomerPortalUrl({
    user,
    returnUrl: returnUrl || `${baseUrl}/dashboard/credits`,
    env,
  });
}
