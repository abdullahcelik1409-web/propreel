...
export function getProviderProductId(packageId, provider = getActivePaymentProvider(), env = process.env) {
  ...
  if (resolvedProvider === "polar") return envValue(POLAR_PRODUCT_ID_ENV_KEYS[packageId], env);
  return null;
}
...
export function getProviderPriceId(packageId, provider = getActivePaymentProvider(), env = process.env) {
  ...
  if (resolvedProvider === "polar") return envValue(POLAR_PRICE_ID_ENV_KEYS[packageId], env) || envValue(POLAR_PRODUCT_ID_ENV_KEYS[packageId], env);
  return null;
}
...
export function getPackageConfig(packageId, provider = getActivePaymentProvider(), env = process.env) {
  ...
  return {
    ...
    variantIdConfigured: resolvedProvider === "paytr" ? true : Boolean(providerPriceId),
