...
function StatusBadge({ status }) {
  ...
  return <span className={`rounded-md border px-2.5 py-1 text-xs font-bold capitalize ${tone}`}>{status}</span>;
}
...
export default function VideoCard({ video }) {
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-[var(--pr-border-soft)] bg-[var(--pr-surface)]">
      <VideoStatusPoller
      ...
      <div className="space-y-4 p-4">
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-[var(--pr-muted)]">
          {current.videoMode && <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">{current.videoMode === "multi_image" ? "Multi Image" : "Basic"}</span>}
          <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">{current.format}</span>
          <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">{current.duration}s</span>
          {Number.isFinite(current.creditsCharged) && <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">{current.creditsCharged} credits</span>}
          {templateName && <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">{templateName}</span>}
          {audioMetadata.audio_status === "completed" && <span className="rounded-md border border-emerald-500/20 px-2 py-1 text-emerald-200">Music</span>}
          {audioMetadata.audio_status === "none" && <span className="rounded-md border border-[var(--pr-border-soft)] px-2 py-1">No Music</span>}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            className="rounded-md border border-red-500/30 px-3 py-2 text-sm font-semibold text-red-200 hover:bg-red-500/10"
          >
            Delete
          </button>
        </div>
